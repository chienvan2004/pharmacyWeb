...
import { getProductWithTag } from '@/services/productsServices'; 
...
export default function ProductTag() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            setLoading(true);
            try {
                const response = await getProductWithTag(); // Thay bằng getProductWithTag
                setProducts(response.data);
            } catch (err: any) {
                setError(err.message || 'Không thể tải sản phẩm theo thẻ');
            } finally {
                setLoading(false);
            }
        };
