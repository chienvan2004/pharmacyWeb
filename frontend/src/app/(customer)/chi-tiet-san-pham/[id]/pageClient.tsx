...
interface ProductDetailPageProps {
    params: { id: string };
    initialProduct?: Product; // Dữ liệu ban đầu từ Server Component
}

export default function ProductDetailPage({ params, initialProduct }: ProductDetailPageProps) {
    const [product, setProduct] = useState<Product | null>(initialProduct || null);
    const [relatedProducts, setRelatedProducts] = useState<Product[]>([]);
    const [loading, setLoading] = useState(!initialProduct); // Chỉ loading nếu chưa có initialProduct
    ...
    // Fetch product data by ID (chỉ chạy nếu chưa có initialProduct)
    useEffect(() => {
        if (initialProduct) return; // Bỏ qua nếu đã có dữ liệu ban đầu
                ...
                {relatedLoading ? (
                    ...
                    <div>
                        ...
                        <div className="flex justify-center items-center gap-4 mt-4">
                            ...
                            <span className="text-gray-700">{currentPage} / {lastPage}</span>
                            <button
                                onClick={() => handlePageChange(currentPage + 1)}
